...
/*   Updated: 2024/06/22 18:57:21 by raveriss         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void BitcoinExchange::processInput(const std::string& filename)
{
    ...
    {
        std::stringstream ss(line);
		// std::cout << "line: " << line[11] << std::endl;
...
double BitcoinExchange::getExchangeRate(const std::string& date) const
{
    ...
    {
        if (it == _exchangeRates.begin()) return 0;
        --it;
    }
    return it->second;
}
...
bool BitcoinExchange::isValidDate(const std::string& date) const
{
    ...
    // Check if all characters in the date parts are digits
    for (int i = 0; i < 4; ++i)
    {
        if (!isdigit(date[i]))
        {
